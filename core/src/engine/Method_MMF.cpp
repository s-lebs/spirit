    ...
    const std::vector<std::shared_ptr<vectorfield>> & configurations, std::vector<vectorfield> & forces )
{
    ...
    if( sparse )
    {
        ...
        successful = Eigenmodes::Sparse_Hessian_Partial_Spectrum(
            this->parameters, image, gradient, hessian, n_modes, basis_3Nx2N, hessian_constrained,
            eigenvalues, eigenvectors );
        std::cout <<"sparse MMF matrix"<< std::endl;
    }
    else
    {
        // The Hessian (unprojected)
    ...
    for (int i=0; i< eigenvalues.size();i++)
      {
	std::cout <<"eigenvalue no. "<<i<<" "<< eigenvalues[i] << std::endl;
      }
