    ...
    bool contributions, bool normalize_by_nos, bool readability_toggle )
{
    ...
    std::string line      = "";
    for( auto & firstcolumn : firstcolumns )
    {
        if( readability_toggle )
            separator += "----------------------++";
        // Centered column titles
        line += fmt::format( " {:^20} ||", firstcolumn );
    }
    if( contributions )
    {
        bool first = true;
    ...
    bool readability_toggle )
{
    ...
    std::string line = fmt::format( " {:^20} || {:^20.10f} |", iteration, system.E * nd );
    for( const auto & pair : system.E_array )
    {
    ...
    bool normalize_by_nos, bool readability_toggle )
{
    ...
    for( std::size_t isystem = 0; isystem < chain.noi; ++isystem )
    {
        auto & system = *chain.images[isystem];
        std::string line
            = fmt::format( " {:^20} || {:^20.10f} || {:^20.10f} |", isystem, chain.Rx[isystem], system.E * nd );
        for( const auto & pair : system.E_array )
        {
            line += fmt::format( "| {:^20.10f} ", pair.second * nd );
        }
    ...
    bool readability_toggle )
{
    ...
    for( std::size_t isystem = 0; isystem < chain.noi; ++isystem )
    {
        ...
        for( std::size_t iinterp = 0; iinterp < chain.gneb_parameters->n_E_interpolations + 1; ++iinterp )
        {
