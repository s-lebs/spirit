...
// Log Precision info
    ...
    // Get parameters
    std::shared_ptr<Data::Parameters_Method_GNEB> params_gneb;
    try
    {
        params_gneb = std::shared_ptr<Data::Parameters_Method_GNEB>(
            IO::Parameters_Method_GNEB_from_Config( state->config_file ) );
    }
    catch( ... )
    {
        spirit_handle_exception_api( -1, -1 );
        params_gneb = std::make_shared<Data::Parameters_Method_GNEB>();
    }

    // Create the chain
    auto images = std::vector<std::shared_ptr<Data::Spin_System>>();
    images.push_back( state->active_image );
    state->chain = std::make_shared<Data::Spin_System_Chain>( images, params_gneb, false );
    //-------------------------------------------------------------------------------
